		...
		for(var i = 0 ; i < json.Entity[0].Instance.length ; i++ ){ 
			...
			if(!marcas[idEvento]){
				...
				marcas[id] = new google.maps.Marker({
					...
					title: titulo,
					icon: foto,
					conteudo:titulo+'<br /><a href="page16.do?kcond5.att19='
								+idEvento+'">aqui</a>'
				});
...
<link href="wr-css/global.css" type="text/css" rel="stylesheet"><link href="wr-css/menu.css" type="text/css" rel="stylesheet"><link href="wr-css/units.css" type="text/css" rel="stylesheet">
  ...
  <body onload="initialize()">
	<div id="_wr_page" title="Eventhis;sv4#page9"></div>
<html:form action="form_page9.do" styleId="page9FormBean" enctype="multipart/form-data">
  <div id="page9HiddenFields" >
                  ...
                  <table border="0" cellspacing="0" align="right">
                    <tr>
                        <td nowrap="nowrap" valign="bottom" style="padding-left:5px">
                          <c:if test="${webratio:isTargetAccessible('ln11', pageContext)}">	
 <a class="shortcuts" href="<webratio:Link link="ln11"/>">
   Log Out
</a>
			</c:if>
                        </td>
 ...
 <a class="shortcuts" href="<webratio:Link link="ln40"/>">
   Ver Favoritos
</a>
			</c:if>
                        </td>
 ...
 <a class="shortcuts" href="<webratio:Link link="ln49"/>">
   Criar Evento
</a>
			</c:if>
                        </td>
                        <td nowrap="nowrap" valign="bottom" style="padding-left:5px">
                          <c:if test="${webratio:isTargetAccessible('ln29', pageContext)}">	
 <a class="shortcuts" href="<webratio:Link link="ln29"/>">
...
<th class=" header" /> 
							...
							<c:forEach var="current" varStatus="status" items="${mciu3.data}">
								<c:set var="index" value="${status.index}" />
									<tr class="row<c:if test="${index mod 2 eq 0}">Alternate</c:if>">	
										<td>
											<html:multibox styleId="mciu3_14150831_${index}" property="mciu3Checked" styleClass="checkBox"><c:out value="${mciu3.key[index]}"/></html:multibox>
										</td>
		...
		<c:if test="${webratio:isTargetAccessible('ln43', pageContext)}">
		 			<a title="Ver Evento Detalhado" id="ln43" class=" linkAlternate" href="<webratio:Link link="ln43" position="index"/>">
					Ver Evento Detalhado</a>
	</c:if>
                                                </c:when>
                                                <c:otherwise>
		<c:if test="${webratio:isTargetAccessible('ln43', pageContext)}">
		 			<a title="Ver Evento Detalhado" id="ln43" class=" link" href="<webratio:Link link="ln43" position="index"/>">
					Ver Evento Detalhado</a>
	</c:if>
                                                </c:otherwise>
                                              </c:choose>
...
<div id="map_canvas" style="width:800px; height:450px; text-align: center; float: left;"></div>
	<table cellspacing="0" id="dadosinit" width="100%" style="border:1px solid #cccccc">
...
<script type="text/javascript">
	$("page9FormBean").observe("wr:pageFocus", function(e) {
