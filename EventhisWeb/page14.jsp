...
<link href="wr-css/global.css" type="text/css" rel="stylesheet"><link href="wr-css/menu.css" type="text/css" rel="stylesheet"><link href="wr-css/units.css" type="text/css" rel="stylesheet">
  ...
  <body>
	<div id="_wr_page" title="Eventhis;sv2#page14"></div>
<html:form action="form_page14.do" styleId="page14FormBean" enctype="multipart/form-data">
  <div id="page14HiddenFields" >
    ...
    <input type="hidden" name="ln12" value="<webratio:Link link="ln12" />">
  </div>
 ...
 <a class="" href="<webratio:Link link="page13.link"/>">
   Login
                      	...
                      	<li class="current">
 <a class="" href="<webratio:Link link="page14.link"/>">
   Pesquisar Eventos
</a>
                  		   <ul>
		                   </ul>
				...
				<table>
					...
					<tr class="row">
						...
						<td valign="middle" nowrap="nowrap" class=" value">
								<html:text onkeypress="if(event.keyCode==13){clickButton('ln12'); return false;}"  size="25" styleId="fld28" styleClass=" field" property="fld28" readonly="false"/>
						</td>
								<td class="error">
<span class=" error"><html:errors property="fld28"/></span>
					...
					<tr class="row">
						...
						<td valign="middle" nowrap="nowrap" class=" value">
								<table>
			<c:forEach var="current" varStatus="status" items="${page14FormBean.map.msfld2List}">
				<tr>
					<td class=" multiselectionfield">
						<html:multibox onkeypress="if(event.keyCode==13){clickButton('ln12'); return false;}" styleClass=" multiselectionfield" property="msfld2" value="${current}" disabled="false" styleId="msfld2_${status.index}"/>
						<c:out value="${page14FormBean.map.msfld2LabelList[status.index]}"/>
					</td>
				</tr>
			</c:forEach>
		</table>
								...
								<td class="error">
<span class=" error"><html:errors property="msfld2"/></span>
</td>
					 	</tr>
					<tr>
						<td colspan="3">
					<table>
				      <tr>
					  <td> 
			<input title="DadosPesquisa" onclick="$('page14FormBean').target='_self'" class=" button"  id="button:ln12" name="button:ln12" type="submit" value="DadosPesquisa">
 </td>
					</tr>
				    </table>
				   </td>
					...
					<c:set var="index" value="${status.index}"/>	
						<tr class="row<c:if test="${pwu1.currentIndex eq index}">Current</c:if><c:if test="${index mod 2 eq 0}">Alternate</c:if>">	
								<td class=" value<c:if test="${pwu1.currentIndex eq index}">Current</c:if><c:if test="${index mod 2 eq 0}">Alternate</c:if>">
									<img src="WRResources/spacer.gif" class="bullet<c:if test="${pwu1.currentIndex eq index}">Current</c:if><c:if test="${index mod 2 eq 0}">Alternate</c:if>" border="0" alt=""/>
								</td>
								   ...
								   <td class=" value<c:if test="${pwu1.currentIndex eq index}">Current</c:if><c:if test="${index mod 2 eq 0}">Alternate</c:if> time">				
										<fmt:formatDate value="${current.dtfim}" pattern="${timePattern}"/>
								   </td>
								   <td class=" value<c:if test="${pwu1.currentIndex eq index}">Current</c:if><c:if test="${index mod 2 eq 0}">Alternate</c:if> time">				
										<fmt:formatDate value="${current.dtinicio}" pattern="${timePattern}"/>
								   </td>
