...
<html:form action="form_page1.do" styleId="page1FormBean" enctype="multipart/form-data">
 ...
 <a class="shortcuts" href="<webratio:Link link="ln6"/>">
   Voltar
</a>
			</c:if>
                        </td>
 ...
 <a class="shortcuts" href="<webratio:Link link="ln8"/>">
   Log Out
</a>
			</c:if>
                        </td>
...
<th class=" header" /> 
			...
			<c:if test="${webratio:isTargetAccessible('ln3', pageContext)}">
<th class=" header" /> 
			</c:if>
								</tr>
							<c:forEach var="current" varStatus="status" items="${mciu1.data}">
								<c:set var="index" value="${status.index}" />
									<tr class="row<c:if test="${index mod 2 eq 0}">Alternate</c:if>">	
										<td>
											<html:multibox styleId="mciu1_11093896_${index}" property="mciu1Checked" styleClass="checkBox"><c:out value="${mciu1.key[index]}"/></html:multibox>
										</td>
		...
		<c:if test="${webratio:isTargetAccessible('ln2', pageContext)}">
		 			<a title="Desmarcar Favorito" id="ln2" class=" linkAlternate" href="<webratio:Link link="ln2" position="index"/>">
					Desmarcar Favorito</a>
	</c:if>
                                                </c:when>
                                                <c:otherwise>
		<c:if test="${webratio:isTargetAccessible('ln2', pageContext)}">
		 			<a title="Desmarcar Favorito" id="ln2" class=" link" href="<webratio:Link link="ln2" position="index"/>">
					Desmarcar Favorito</a>
	</c:if>
                                                </c:otherwise>
                                              </c:choose>
			...
			<c:if test="${webratio:isTargetAccessible('ln3', pageContext)}">
                                          <td> 
                                              <c:choose>
                                                <c:when test="${index mod 2 eq 0}">
		<c:if test="${webratio:isTargetAccessible('ln3', pageContext)}">
		 			<a title="Ver Evento Detalhado" id="ln3" class=" linkAlternate" href="<webratio:Link link="ln3" position="index"/>">
					Ver Evento Detalhado</a>
	</c:if>
                                                </c:when>
                                                <c:otherwise>
		<c:if test="${webratio:isTargetAccessible('ln3', pageContext)}">
		 			<a title="Ver Evento Detalhado" id="ln3" class=" link" href="<webratio:Link link="ln3" position="index"/>">
					Ver Evento Detalhado</a>
	</c:if>
                                                </c:otherwise>
                                              </c:choose>
