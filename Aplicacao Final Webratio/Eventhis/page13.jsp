...
<link href="wr-css/global.css" type="text/css" rel="stylesheet"><link href="wr-css/menu.css" type="text/css" rel="stylesheet"><link href="wr-css/units.css" type="text/css" rel="stylesheet">
  ...
  <body>
	<div id="_wr_page" title="Eventhis;sv4#area2#page13"></div>
<html:form action="form_page13.do" styleId="page13FormBean" enctype="multipart/form-data">
  <div id="page13HiddenFields" >
 ...
 <a class="shortcuts" href="<webratio:Link link="ln70"/>">
   Voltar
</a>
			</c:if>
                        </td>
                        <td nowrap="nowrap" valign="bottom" style="padding-left:5px">
                          <c:if test="${webratio:isTargetAccessible('ln71', pageContext)}">	
 <a class="shortcuts" href="<webratio:Link link="ln71"/>">
   Log Out
</a>
			</c:if>
                        </td>
...
<c:if test="${not(empty mciu2) and (mciu2.dataSize gt 0)}">
    ...
    <td rowspan="1" colspan="1">
		<div class="plain ">
			<div class="plain MultiChoiceIndexUnit">
				<table>
					<tr>
						<td colspan="2" class="error"><html:errors property="mciu2Checked" /></td>
					</tr>
					<tr>
						<td>
...
<th class=" header" /> 
			...
			<c:if test="${webratio:isTargetAccessible('ln68', pageContext)}">
<th class=" header" /> 
			</c:if>
								</tr>
							<c:forEach var="current" varStatus="status" items="${mciu2.data}">
								<c:set var="index" value="${status.index}" />
									<tr class="row<c:if test="${index mod 2 eq 0}">Alternate</c:if>">	
										<td>
											<html:multibox styleId="mciu2_448291_${index}" property="mciu2Checked" styleClass="checkBox"><c:out value="${mciu2.key[index]}"/></html:multibox>
										</td>
		...
		<c:if test="${webratio:isTargetAccessible('ln67', pageContext)}">
		 			<a title="Desmarcar Favorito" id="ln67" class=" linkAlternate" href="<webratio:Link link="ln67" position="index"/>">
					Desmarcar Favorito</a>
	</c:if>
                                                </c:when>
                                                <c:otherwise>
		<c:if test="${webratio:isTargetAccessible('ln67', pageContext)}">
		 			<a title="Desmarcar Favorito" id="ln67" class=" link" href="<webratio:Link link="ln67" position="index"/>">
					Desmarcar Favorito</a>
	</c:if>
                                                </c:otherwise>
                                              </c:choose>
			...
			<c:if test="${webratio:isTargetAccessible('ln68', pageContext)}">
                                          <td> 
                                              <c:choose>
                                                <c:when test="${index mod 2 eq 0}">
		<c:if test="${webratio:isTargetAccessible('ln68', pageContext)}">
		 			<a title="Ver Evento Detalhado" id="ln68" class=" linkAlternate" href="<webratio:Link link="ln68" position="index"/>">
					Ver Evento Detalhado</a>
	</c:if>
                                                </c:when>
                                                <c:otherwise>
		<c:if test="${webratio:isTargetAccessible('ln68', pageContext)}">
		 			<a title="Ver Evento Detalhado" id="ln68" class=" link" href="<webratio:Link link="ln68" position="index"/>">
					Ver Evento Detalhado</a>
	</c:if>
                                                </c:otherwise>
                                              </c:choose>
...
<script type="text/javascript">
	$("page13FormBean").observe("wr:pageFocus", function(e) {
